...
// receive packets
Coroutine receive(Buffer &radioBuffer, Buffer &pcapBuffer) {
	bool headerWritten = false;
	uint32_t startTime = -1;
	while (true) {
		// wait until USB device gets detected (radio buffer becomes ready)
		std::cout << "Wait for USB radio device..." << std::endl;
		...
		while (radioBuffer.ready()) {
			...
			// get timestamp
			uint8_t const *ts = &radioBuffer.data()[-4];
			...
			if (pcapBuffer.ready()) {
				...
				if (!headerWritten) {
					...
					co_await pcap::writeHeader(pcapBuffer, header, error);
				...
				co_await pcap::writePacket(pcapBuffer, header, radioBuffer.array<const uint8_t>(), error);
...
Coroutine replay(Loop &loop, Buffer &pcapBuffer) {
	// wait until pcap file is open or opening of the file fails
	co_await select(pcapBuffer.untilReady(), pcapBuffer.untilDisabled());
	if (!pcapBuffer.ready()) {
	...
	pcap::Header header;
	bool error;
	co_await pcap::readHeader(pcapBuffer, header, error);
	...
	if (error) {
		...
		while (true) {
			...
			co_await pcap::readPacket(pcapBuffer, header, packet, error);

			if (error)//pcapReader.error())
				break;
...
int main(int argc, char const *argv[]) {
	...
	if (inputFile.empty()) {
		...
		Drivers drivers;
		if (!outputFile.empty()) {
			std::cout << "Write packets to " << outputFile.string() << std::endl;
			drivers.file.open(outputFile, File::Mode::WRITE | File::Mode::TRUNCATE);
		}

		// receive from radio (pan and long/short address have no effect)
		...
		drivers.radio.open(radioChannel);

		// receive from radio
		receive(drivers.radioBuffer, drivers.fileBuffer);
		drivers.loop.run();
	} else {
		// replay from file

		DriversReader drivers;
		drivers.file.open(inputFile, File::Mode::READ);

		// replay from pcap
		replay(drivers.loop, drivers.fileBuffer);
		drivers.loop.run();
	}
	return 0;
}
